...
class Encoder(nn.Module):
    def __init__(self, cin, cout, nf=64, activation=nn.Tanh):
        ...
        network = [
            nn.Conv2d(cin, nf, kernel_size=8, stride=4, padding=2, bias=False),  # 256x256 -> 64x64
            nn.ReLU(inplace=True),
            nn.Conv2d(nf, nf * 2, kernel_size=8, stride=4, padding=2, bias=False),  # 64x64 -> 16x16
            nn.ReLU(inplace=True),
...
if __name__ == '__main__':
    ...
    network = ConvLayer(32, 32).to(device)
    ...
    # light network
    # netL = Encoder(cin=3, cout=2, nf=32).to(device)
